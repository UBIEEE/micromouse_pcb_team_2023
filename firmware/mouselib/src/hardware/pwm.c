...
result_t pwm_set_freq_duty_cycle(uint slice_num, uint channel, uint32_t freq,
                                 uint32_t duty_cycle) {
  uint32_t divider16 = MICROMOUSE_CLOCK_FREQ_HZ / freq / 4096 +
                       (MICROMOUSE_CLOCK_FREQ_HZ % (freq * 4096) != 0);
