...
#include <micromouse/utils/general_utils.h>
...
result_t drv8835_init(drv8835_dev_t* dev, uint8_t a_phase_pin, uint8_t a_enable_pin,
  ...
  result_t result = drv8835_set_output(dev, 0.f, 0.f);
  return result;
}

result_t drv8835_set_output(drv8835_dev_t* dev, float a_percent, float b_percent) {
  if (!dev) {
    return RESULT_INVALID_ARG;
  }
  ...
  result = pwm_set_freq_duty_cycle(dev->a_phase_slice_num, dev->a_phase_channel, DRV8835_FREQ_HZ,
                                   a_duty_cycle);
  result = pwm_set_freq_duty_cycle(dev->b_phase_slice_num, dev->b_phase_channel, DRV8835_FREQ_HZ,
                                   b_duty_cycle);
