...
drv8835_dev_t drv8835_init(uint8_t a_phase_pin, uint8_t a_enable_pin,
  ...
  drv8835_set_output(&dev, 0.f, 0.f);
...
void drv8835_deinit(drv8835_dev_t* dev) {
  ...
  // Disable PWM.
  pwm_set_enabled(dev->a_phase_slice_num, false);
  pwm_set_enabled(dev->b_phase_slice_num, false);

  // Disable motor driver.
  set_enable(dev, false, false);
}
...
void drv8835_set_output(drv8835_dev_t* dev, float a_percent, float b_percent) {
  ...
  const uint32_t a_duty_cycle = (uint32_t)((a_percent + 1.f) * 50.f);
  const uint32_t b_duty_cycle = (uint32_t)((b_percent + 1.f) * 50.f);
  ...
  const bool a_enable = !FLOAT_EQ(a_percent, 0.f);
  const bool b_enable = !FLOAT_EQ(b_percent, 0.f);
